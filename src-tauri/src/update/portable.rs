...
mod tests {
    ...
    // 7a. cleanup_stale_old apaga o .old quando ele existe.
    #[test]
    fn cleanup_stale_old_apaga_o_old_quando_existe() {
        let exe_dir = tempfile::tempdir().unwrap();
        let old_path = exe_dir.path().join("app.exe.old");
        fs::write(&old_path, b"lixo").unwrap();

        cleanup_stale_old(exe_dir.path(), "app.exe").unwrap();

        assert!(!old_path.exists());
    }

    // 7b. cleanup_stale_old é no-op silencioso quando não há .old.
    #[test]
    fn cleanup_stale_old_e_no_op_quando_nao_ha_old() {
        let exe_dir = tempfile::tempdir().unwrap();
