...
| TERM-06 | P1: Header (git, rename manual) | Tasks | **Parcial, corrigido de verdade na triagem 008 (11/08/2026)**: a parte de **rename manual** (`T16`) tem código real (`terminal_set_title` no backend, edição por duplo-clique no `TerminalHeader`) e agora integrado ponta a ponta — `TerminalPane` reporta o id REAL da sessão (o `TerminalId` de `pty_spawn`, não o UUID de `createTerminalId()` usado só como chave de grid) via a prop `onSessionId`, e `App.tsx` passa esse id (não `terminal.id`) para `TerminalHeader`, então o rename manual grava na mesma chave que o agente usa via MCP e a regra "rename manual vence o agente" é de fato exercitada. `cargo build`, `npm run build` e `npm run test -- --run` (72/72, 15 arquivos) verdes após a correção. A parte de **branch/git no header** continua sem task, não agendada |
...
| TERM-10 | P1: Seletor de pasta ao criar terminal | Tasks | **Done, corrigido na triagem 008 (11/08/2026)** — `T14, T15` implementadas: `NewTerminalDialog.tsx` usa `open({directory:true})` do `tauri-plugin-dialog`, campo somente-leitura, 6/6 testes passando. **Contexto de 03/08/2026 permanece**: também alcançado via "Import Project" no picker de projetos (`projects/spec.md`, PROJ-06) |
| TERM-11 | P1: Lembrar o último diretório escolhido | Tasks | **Done, corrigido na triagem 008 (11/08/2026)** — `T13, T14, T15` implementadas: `picker_prefs.rs` (upsert de linha única), comandos `terminal_picker_last_dir`/`terminal_picker_set_last_dir` registrados em `lib.rs`, 4/4 testes passando |
| TERM-12 | P1: Restaurar sessão após fechamento inesperado | Tasks | **Novo 03/08/2026** — Pending |

**Cobertura (corrigida na triagem 008, 11/08/2026 — a tabela anterior dizia "7 mapeados", defasada por `TERM-10`/`TERM-11` terem sido implementadas entre a triagem 007 e esta):** 10 requisitos, **9 mapeados e implementados** (`TERM-01..05, 07, 08, 10, 11`; `TERM-06` conta como parcial, não "sem cobertura" — tem código real, com o gap de integração de `App.tsx`/`TerminalPane.tsx` fechado nesta mesma triagem), **2 sem cobertura nenhuma** (`TERM-09`, `TERM-12`) ⚠️. Note também que `T7, T9, T10, T11` — que cobrem `TERM-01, 02, 04, 05, 07, 08` — têm `Verify` visual não confirmável enquanto o NEEDS-DECISION de integração de `App.tsx` (ver `tasks.md`) não for resolvido: "Done" aqui significa gate automatizado verde, não confirmado no app real.

**`TERM-10`/`TERM-11` implementadas entre a triagem 007 (03/08/2026) e a 008 (11/08/2026)** — mapeadas para `T13, T14, T15` em `tasks.md`, confirmado por leitura de código e teste real (não presumido).
