...
|---|---|---|---|
| REL-01 | P1: Release só por `workflow_dispatch` com select de incremento | Execute | **Verified, corrigido na triagem 008 (11/08/2026)**: gate `pipeline` real confirmado — `v0.1.1` (03/08, `30778409874`) e `v0.1.2` (03/08, `30807513912`), ambas via `workflow_dispatch`, `gh run list --workflow=release.yml` |
| REL-02 | P1: Push/tag/agendamento nunca publicam | Execute | **Verified, corrigido na triagem 008 (11/08/2026)**: as duas releases reais só dispararam por `workflow_dispatch`; nenhuma tag/push publicou sozinha |
...
| REL-05 | P1: CHANGELOG dos Conventional Commits desde a última tag | Execute | **Verified em produção, corrigido na triagem 008 (11/08/2026)**: `v0.1.1`/`v0.1.2` publicadas com sucesso via o mesmo job que invoca `git-cliff` (`release.yml:100-125`) — se o passo tivesse falhado, o release inteiro teria quebrado. Ressalva que continua válida: `git-cliff` segue **não instalado localmente** (`T3`, `git cliff --version` falha no ambiente do mantenedor) — a prova é indireta (CI passou), não uma inspeção manual do CHANGELOG gerado |
| REL-06 | P1: Commit `chore(release)`, tag e GitHub Release no mesmo run | Execute | ✅ **Corrigido e confirmado em produção, triagem 006.** O bug de `git add` (`release.yml:135` referenciando `src-tauri/Cargo.toml` inexistente) foi corrigido no commit `bc7ab64` ("Fix release workflow to correctly add files for commit"). A prova não é só leitura: a `Release v0.1.1` rodou via `workflow_dispatch` em produção e **teve sucesso** (`gh run list` → `30778409874`, 03/08/2026, tag `v0.1.1` publicada em `origin/master`) — ver `project/STATE.md` |
...
| REL-08 | P1: Run interrompido desfaz tag, rascunho e commit de versão | Execute | Implemented — parcial: job `cleanup` (`release.yml:282-333`, T11) confirmado por leitura; o cenário de FALHA (disparo cancelado de propósito) segue **não testado** — as duas releases reais foram sucessos, não exercitam o `cleanup`. Único item desta linha ainda genuinamente pendente de verificação |
| REL-09 | P1: `.msi` + `-setup.exe` + `.deb` + `.AppImage` em toda release | Execute | **Verified em produção, corrigido na triagem 008 (11/08/2026)**: `gh release view v0.1.2 --json assets` confirma os 5 artefatos (`.msi`, `-setup.exe`, `.deb`, `.AppImage`, zip portátil) + `.sig` de cada um + `latest.json` |
| REL-10 | P1: NSIS em `currentUser`, sem UAC | Execute | Implemented (T4, `src-tauri/tauri.conf.json:39-41`) |
| REL-11 | P1: Artefatos de update assinados + `latest.json` | Execute | **Verified em produção, corrigido na triagem 008 (11/08/2026)**: `latest.json` real de `v0.1.2` confirmado no release do GitHub, com `.sig` em cada plataforma |
...
| REL-13 | P1: Build de Linux em `ubuntu-22.04` | Execute | Implemented (T9, `release.yml:155` — linha corrigida na triagem 005, a citação antiga apontava para um comentário, não para a entrada da matriz — mesma base do job `rust` do `ci.yml`) |
| REL-14 | P1: `.zip` portátil de Windows com marcador | Execute | ✅ **Corrigido, triagem 006.** O bug do caminho *default* do binário (`achado na triagem 005`) foi corrigido — `defaultBinaryPath()` resolve sob `<raiz>/target/release/`, com 2 testes novos (8/8, era 6/6). A `Release v0.1.1` publicada em produção (ver REL-06) é evidência indireta de que o pipeline completo, incluindo este script, roda de ponta a ponta. Ver `tasks.md` T7 |
| REL-15 | P1: Portátil assinado com a mesma chave, `.sig` anexado | Execute | **Verified em produção, corrigido na triagem 008 (11/08/2026)**: `SwarmDeck_0.1.2_x64-portable.zip.sig` existe no release real, gerado pela mesma chave dos demais artefatos |
...
| REL-18 | P1: Rodar do zip em pasta gravável, sem elevação | Execute | Implemented — parcial: detecção de pasta somente-leitura (`is_writable`) coberta por `T13`; montagem do zip corrigida e testada (T7, `npm run test:scripts` 8/8, era 6/6 — triagem 006); rodar de fato o executável descompactado e clicar nele continua sendo verificação humana, não coberta por nenhuma run automatizada |
| REL-19 | P1: Verificação silenciosa no boot, sem bloquear a UI | Execute | Implemented (T14, `src-tauri/src/update/check.rs`, testado) — **corrigido na triagem 006**: dizia `Design/Pending`, o código já existe e passa |
| REL-20 | P1: Aviso não bloqueante com versão, notas e 3 ações | Execute | Implemented (T15/T17, `UpdateBanner.tsx`, 9 testes Vitest) — corrigido na triagem 006 |
| REL-21 | P1: Nada é exibido quando não há versão nova | Execute | Implemented (T14/T15, testado) — corrigido na triagem 006 |
| REL-22 | P1: Download com progresso, assinatura verificada, aplica e reinicia | Execute | Implemented — parcial: modo instalado via plugin oficial (T14); modo portátil (T16, `update/portable.rs`, 9 testes); relançamento real não automatizável, verificação humana declarada (T19) — corrigido na triagem 006 |
| REL-23 | P1: "Pular esta versão" vale só para aquela versão | Execute | Implemented (T14, `db/settings.rs::skip_version`, testado) — corrigido na triagem 006 |
| REL-24 | P1: Instalado usa o plugin oficial; portátil troca arquivos sem elevação | Execute | Implemented (T14 + T16) — corrigido na triagem 006 |
| REL-25 | P1: Assinatura inválida aborta e preserva a versão atual | Execute | Implemented (T16, `update/portable.rs`, testes de assinatura adulterada) — corrigido na triagem 006 |
| REL-26 | P1: Aviso de encerramento dos PTYs antes de reiniciar | Execute | Implemented (T18, `commands::update::terminals_active_count`) — corrigido na triagem 006 |
| REL-27 | P2: CI roda `npm run build`, `npm run test` e `cargo test` em push e PR | Execute | Implemented (`.github/workflows/ci.yml`, jobs `frontend` e `rust`) — corrigido na triagem 006, confirmado por leitura e por gate medido nesta triagem |
| REL-28 | P2: Job Linux instala as dependências de sistema do Tauri (sem `protoc`) | Execute | Implemented (`ci.yml`, jobs `rust`/`clippy`, `apt-get install` sem `protoc` na lista) — corrigido na triagem 006 |
| REL-29 | P2: Conventional Commits validado nos commits do PR | Execute | Implemented (`ci.yml`, job `commits`, `if: github.event_name == 'pull_request'`) — corrigido na triagem 006 |
| REL-30 | P2: `cargo fmt --check` e testes dos scripts de release no CI | Execute | Implemented (`ci.yml`, `cargo fmt --all -- --check` no job `rust`, `npm run test:scripts` no job `frontend`) — corrigido na triagem 006 |
| REL-31 | P2: Concorrência cancela execução superada; CI não publica nada | Execute | Implemented (`ci.yml:14-16`, `concurrency.cancel-in-progress: true`; `release.yml` não tem gatilho `push`/`pull_request`, só `workflow_dispatch`) — corrigido na triagem 006 |
| REL-32 | P2: Seção "Atualizações" com versão e modo | Execute | Implemented (T17, `UpdateSettings.tsx`) — corrigido na triagem 006. **Ressalva nova, triagem 006**: `UpdateSettings.tsx` nunca é montado por nenhuma janela/rota real — mesmo gap de Settings inatingível que afeta `agent-selection`/`projects`, ver `project/STATE.md` |
| REL-33 | P2: "Verificar agora" informa o resultado nos dois casos | Execute | Implemented (T17, testado) — corrigido na triagem 006, mesma ressalva de alcance real acima |
| REL-34 | P2: Toggle persistido; desligado = zero rede no boot; ligado por padrão | Execute | Implemented (T14, `db/settings.rs::auto_check`) — corrigido na triagem 006, mesma ressalva de alcance real acima |
...
| REL-36 | P3: `clippy -D warnings` no CI após limpeza | Execute | Implemented (`.github/workflows/ci.yml` job `clippy`, `cargo clippy --all-targets -- -D warnings`) — corrigido na triagem 006: a linha dizia `Pending` sem fase, mas o job já existe no workflow e `cargo clippy --all-targets -- -D warnings` mede limpo agora (0 warnings) |

**Cobertura:** 36 requisitos, 36 mapeados para tarefas em `tasks.md`.

**Reescrita quase inteira na triagem 006 (03/08/2026).** Esta tabela vinha de duas triagens atrás (28 requisitos ainda em `Design/Pending` sem nenhum código citado, apesar de `tasks.md` ter T1-T20 majoritariamente implementadas e testadas) e envelheceu mais uma vez desde então: os dois bugs reais que a triagem 005 achou (REL-06: `git add` de caminho inexistente; REL-14/REL-18: caminho *default* do binário errado) **foram corrigidos** — não é presunção, é confirmado por uma **`Release v0.1.1` real, publicada em produção** via `workflow_dispatch` com sucesso em 03/08/2026 (`gh run list`, ver `project/STATE.md`). Estado agora:

- **Implemented, sem ressalva** (gate verde, sem bug conhecido, sem gap de alcance): REL-03, REL-04, REL-06, REL-07, REL-10, REL-12, REL-13, REL-14, REL-16, REL-17, REL-18, REL-19, REL-21, REL-23, REL-24, REL-25, REL-26, REL-27, REL-28, REL-29, REL-30, REL-31, REL-35, REL-36 — **24**
- **Implemented, alcance real confirmado na triagem 008 (11/08/2026)**: `settings-shell/T2` fechou entre a triagem 007 e esta — `UpdateSettings.tsx` agora é montado de verdade pela janela `settings`. REL-20, REL-32, REL-33 — **3**. `REL-34` (toggle persistido) fica à parte: `db::auto_check`/`set_auto_check` existe no backend, mas **nenhum comando Tauri o expõe** (`grep -rn "auto_check" src-tauri/src/commands/` → vazio) — o toggle na UI não persiste. Ver `Fora da execução` desta triagem.
- **Verified em produção (triagem 008, 11/08/2026)**: duas releases reais (`v0.1.1`, `v0.1.2`) confirmam REL-01, REL-02, REL-05 (indireto), REL-09, REL-11, REL-15 — **6**. REL-08 (cenário de falha do `cleanup`) e REL-22 (aplicação real do update, `T19`) seguem sem prova — nenhuma release real exercitou o caminho de erro nem uma máquina rodou o updater de ponta a ponta.

Soma: 24 + 3 + 6 + 2 (REL-08, REL-22, não verificados) + 1 (REL-34, gap real) = 36. **Nenhum requisito permanece `Pending` sem código.** `tasks.md` (`T2, T5, T6, T9, T10, T11, T12, T21`) ainda descreve blocos como "BLOQUEADA pendente de T12/T5" — **corrigido nesta triagem**, ver `tasks.md`.
