...
- **Implemented, com ressalva de alcance real** (a UI existe e testa, mas nenhuma janela/rota monta `UpdateSettings.tsx` — mesmo gap de Settings inatingível que afeta `agent-selection`/`projects`, ver `project/STATE.md`): REL-20, REL-32, REL-33, REL-34 — **4**
- **Implemented — parcial**, pendente de passo humano específico (`T5` chave de assinatura, `T19` verificação ponta a ponta de update real): REL-01, REL-02, REL-05, REL-08, REL-09, REL-11, REL-15, REL-22 — **8**

Soma: 24 + 4 + 8 = 36. **Nenhum requisito permanece `Pending` sem código.** Ressalva importante desta triagem: a correção acima foi feita linha a linha contra o código e contra o `gh run list` real, mas **não** contra uma reconciliação task a task de `tasks.md` (que ainda descreve `T5`-`T12`/`T19` com o texto de antes do release ter saído) — ver a AD de 03/08/2026 em `project/STATE.md` para o que ficou como "não verificado".
