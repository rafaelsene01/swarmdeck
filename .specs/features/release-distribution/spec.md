...
| REL-06 | P1: Commit `chore(release)`, tag e GitHub Release no mesmo run | Execute | 🐛 **Bug real, achado na triagem 005** — o `git add` do commit (`release.yml:135`) referencia `src-tauri/Cargo.toml`/`src-tauri/Cargo.lock`, que não existem (a versão mora no `Cargo.toml` da raiz). Sob `set -euo pipefail`, o job falha antes de commitar/taguear — nem o commit, nem a tag, nem o push acontecem num disparo real. Ver `tasks.md` T6. Não é "pendente de T12", é "quebraria em T12" |
...
| REL-14 | P1: `.zip` portátil de Windows com marcador | Execute | Implemented — parcial, 🐛 achado na triagem 005: as funções puras (nomeação, `stageBundle` com `binary` explícito) passam 6/6. Mas o caminho *default* do binário (usado pelo `release.yml`, que nunca passa `--binary`) resolve para `src-tauri/target/release/`, e um build real do workspace sai em `<raiz>/target/release/` — o script falharia com "binary not found" num disparo real. Ver `tasks.md` T7 |
...
**Cobertura:** 36 requisitos, 36 mapeados para tarefas em `tasks.md` (ver "Cobertura de requisitos" nesse arquivo). **Corrigido na triagem 005 (02/08/2026), depois de um auditor somente-leitura verificar `T6`–`T11` linha a linha contra `release.yml`:** 2 bugs reais achados, não meras pendências de execução — REL-06 (`git add` de arquivo inexistente, deriva o job `prepare`) e REL-14/REL-18 (caminho default do binário em `make-portable.mjs` não bate com o layout do workspace Cargo). Como `T9`, `T10`, `T11` dependem de `T6` (`needs: prepare`), **o bug de REL-06 bloqueia toda a Fase C**, não só a falta de `T12`/`T5`. Requisitos com evidência de gate local verde e sem bug conhecido (REL-03, REL-04, REL-07, REL-10, REL-12, REL-13, REL-16, REL-17, REL-35): **9**. `Implemented — parcial`, sem bug conhecido, pendente só de `T12`/`T5` (REL-01, REL-02, REL-05, REL-09, REL-11, REL-15): **6**. Com bug real achado nesta triagem (REL-06, REL-14, REL-18): **3**. Os demais 18 permanecem `Pending`: REL-19–REL-26 (update no app, fora do escopo de `T6`-`T11`), REL-32–REL-34 (seção "Atualizações"), REL-36 (`clippy` no CI) e REL-27–REL-31 — este bloco já tem código escrito e `Done when` majoritariamente `[x]` em `T2`/`tasks.md`, mas esta tabela não foi atualizada; pendência textual separada, não requisito sem código.
