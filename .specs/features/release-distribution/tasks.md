...
> ✅ **Bug real corrigido nesta run (era 🐛 BLOQUEADA — achado do auditor na triagem 005, 02/08/2026).** O step "Commitar e taguear" (`release.yml:135`) fazia `git add package.json package-lock.json src-tauri/Cargo.toml src-tauri/Cargo.lock CHANGELOG.md` — mas **`src-tauri/Cargo.lock` não existe** (confirmado por `ls`: workspace tem um único lockfile, na raiz; `src-tauri/Cargo.toml` existe, mas só herda a versão via `version.workspace = true` — quem grava a versão de verdade é o `Cargo.toml` da raiz, via `bump-version.mjs`). O lockfile do workspace é `Cargo.lock` na raiz, atualizado por `cargo metadata --manifest-path src-tauri/Cargo.toml` (`release.yml:98`), que resolve o workspace e escreve no `Cargo.lock` raiz — não em `src-tauri/`. `T1` (`bump-version.mjs`) já escreve no lugar certo; este `git add`, em `T6`, ainda copiava o caminho do `local-mind` (repo de referência), que não tinha sido adaptado. Sob `set -euo pipefail`, um `git add` com pathspec inexistente falha com exit≠0 — o job `prepare` quebraria **no primeiro disparo real**, antes de qualquer commit, tag ou push. Como `T9`, `T10`, `T11` dependem de `T6` (`needs: prepare`), **a cadeia inteira ficava inalcançável por este bug**, não só por faltar `T12` (clique humano) ou `T5` (secrets). **Correção aplicada**: `release.yml:135` agora faz `git add package.json package-lock.json Cargo.toml Cargo.lock CHANGELOG.md` (caminhos da raiz). Verificado por leitura + `ls` (sem disparo real do workflow) — o `Done when` abaixo ainda cobra o disparo real (`T12`), que este fix não substitui.
...
- [x] Commit `chore(release): vX.Y.Z` incluindo `package.json`, `package-lock.json`, `Cargo.toml`, `Cargo.lock`, `CHANGELOG.md` — confirmado em `release.yml:135` (bug de pathspec inexistente achado na triagem 005 e corrigido nesta run — ver callout ✅ acima; item ficou riscado/`[ ]` entre a triagem 005 e esta correção)
- [x] Output `release_sha` exposto para o `cleanup` — confirmado em `release.yml:35` (`outputs.release_sha`) e `:142` (`echo "sha=..."`)
- [ ] Gate passa: disparo real com resultado esperado — o bug de `git add` (pathspec inexistente) já foi corrigido nesta run e não bloqueia mais; falta só `T12` (verificação humana, disparo real) — item permanece `[ ]` até esse disparo acontecer
