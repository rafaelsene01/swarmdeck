...
| **scripts** | `npm run test:scripts` | Tarefa que toca os scripts de release em `scripts/*.mjs`. **Existe desde `release-distribution/T1` (31/07/2026)**: roda `node --test "scripts/**/*.test.mjs"`, o test runner nativo do Node 24. Medido na triagem 004 (01/08/2026): **11 passed / 0 failed**. As outras tarefas com este gate são `rd/T7` e `rd/T8`. |
...
`cargo test --lib` roda só os testes unitários in-crate (rápido). `cargo test` inclui `tests/` — os de integração, que tocam disco e socket. `npm run test:scripts` roda `node --test "scripts/**/*.test.mjs"`, o test runner nativo do Node 24 — o glob explícito é necessário porque `node --test scripts/` (sem glob) casa a própria pasta como arquivo de teste e falha (`release-distribution/tasks.md` → "Desvios de execução", T1).
